...
import { MAX_FILE_SIZE_BYTES, DEFAULT_BLOCKED_DIRECTORIES, HIGH_RISK_EXTENSIONS } from '../constants.js';
import { scanForInjection, hashContent, createFileMetadata, addAuditEntry, createSecurityAlert } from './security.js';
import { createApprovalRequest, getApprovalRequest } from './approval.js';
 ...
 */
export function setAccessPolicy(policy) {
    accessPolicy = {
        ...accessPolicy,
        ...policy
    };
 ...
 */
export function isPathSensitive(path) {
 ...
 */
export async function secureReadFile(path, scanForInjectionPatterns = true) {
    const normalizedPath = normalize(resolve(path));
    // Check access policy
 ...
 */
export async function secureWriteFile(path, content, requireApproval = true) {
    const normalizedPath = normalize(resolve(path));
    // Check access policy
    const pathCheck = isPathAllowed(normalizedPath);
    if (!pathCheck.allowed) {
    ...
    if (requireApproval || isSensitivePath) {
        const approval = createApprovalRequest('write_file', `Write ${contentSize} bytes to ${normalizedPath}`, {
            path: normalizedPath,
            contentSize,
            contentHash: hashContent(content),
            isSensitivePath,
 ...
 */
export async function executeApprovedWrite(approvalId, path, content) {
    // Verify approval
